...
#define PROXIMITY_MAX_REGISTERS   0x4D
...
#define PIC12LF1822_UARTLOG       0x4C
...
static ssize_t prox_uartlog_show(struct sys_device *dev, struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	mx50_proximity_read_i2c(&value, PIC12LF1822_UARTLOG);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_uartlog_store(struct sys_device *dev, struct sysdev_attribute *attr,
					const char *buf, size_t size)
{
	u8 value = 0;
	...
	mx50_proximity_write_i2c(PIC12LF1822_UARTLOG, value);
	return size;
}

static SYSDEV_ATTR(uartlog, 0644, prox_uartlog_show, prox_uartlog_store);
...
static int mx50_proximity_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (error) goto exit;
	error = sysdev_create_file(&device_proximity, &attr_uartlog);
	if (error) goto exit;
...
static int mx50_proximity_remove(struct i2c_client *client)
{
	...
	sysdev_remove_file(&device_proximity, &attr_uartlog);
