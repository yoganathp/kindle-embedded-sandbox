				   ...
				   struct mxc_epdc_fb_data *fb_data)
{
	...
	if (upd_desc_list->upd_data.waveform_mode == WAVEFORM_MODE_AUTO)
	{
		...
		if (mxc_epdc_debugging) {
			printk(KERN_INFO "WAVEFORM_AUTO: PXP returns hist_stat = 0x%x and waveform is 0x%x\n", hist_stat, upd_desc_list->upd_data.waveform_mode);
		}
				...
				struct update_desc_list *update_to_merge)
{
	...
	if (mxc_epdc_debugging)
	{
		printk(KERN_INFO "Going to merge update %d and %d. Waveforms: %d and %d\n", 
	...
	if (mxc_epdc_debugging)
	{
		printk(KERN_INFO "Merging update %d and %d. New waveform: %d\n", 
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	if (upd_data_list->update_desc->upd_data.temp == TEMP_USE_PAPYRUS) {
				   ...
				   struct fb_info *info)
{
	...
	if(mxc_epdc_debugging)
	{
		printk(KERN_INFO "Requested waveforms: mode: 0x%x __ BW: 0x%x __ Gray : 0x%x\n", 
				upd_data->waveform_mode,
	...
	if(mxc_epdc_debugging)
	{
		printk(KERN_INFO "Converted waveforms: mode: 0x%x __ BW: 0x%x __ Gray : 0x%x\n", 
				upd_data->waveform_mode,
				upd_data->hist_bw_waveform_mode,
				upd_data->hist_gray_waveform_mode);
	}
	...
	if (upd_data->update_marker != 0) {
		...
		if (mxc_epdc_debugging) {
			...
			printk(KERN_INFO "mxc_epdc: waveform=0x%x mode=0x%x temp:%d update region top=%d, left=%d, width=%d, heigth=%d\n", upd_data->waveform_mode, upd_data->update_mode, papyrus_temp, upd_data->update_region.top, upd_data->update_region.left, upd_data->update_region.width, upd_data->update_region.height);
		}
	}
	...
	if (upd_desc->upd_data.temp == TEMP_USE_PAPYRUS) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			papyrus_temp);
		epdc_set_temp(temp_index);
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	if (fb_data->cur_update->update_desc->upd_data.temp == TEMP_USE_PAPYRUS) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			papyrus_temp);
    epdc_set_temp(temp_index);
...
*/
	...
	if(use_builtin_cmap == 1) {
		dev_err(&pdev->dev, "use_builtin_cmap !!\n");
		...
		dev_err(&pdev->dev, "not use_builtin_cmap !!\n");	
